...
public class ComboBoxComponent<E> extends JComboBox<E> {
    ...
    private class CustomComboBoxRenderer extends DefaultListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            label.setBorder(BorderFactory.createEmptyBorder(5, 10, 5, 10));  // Padding inside each item
            label.setOpaque(true);

            if (isSelected) {
                label.setBackground(new Color(0, 111, 238));  // Blue background when selected
                label.setForeground(Color.WHITE);  // White text when selected
            } else {
                label.setBackground(new Color(255, 255, 255));  // White background when not selected
    ...
    private class CustomComboBoxUI extends javax.swing.plaf.basic.BasicComboBoxUI {
        ...
        public void installUI(JComponent c) {
            ...
            comboBox.setBorder(BorderFactory.createLineBorder(new Color(0, 111, 238), 2));  // Blue border
